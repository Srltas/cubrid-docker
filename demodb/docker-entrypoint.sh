...
# Create demodb
seed_demodb() {
    if [ -f "$CUBRID_DATABASES/databases.txt" ] && grep -qw '^demodb' "$CUBRID_DATABASES/databases.txt"; then
        echo "Demodb already exists (databases.txt)"
        return 0
    fi

    if [ -x "$CUBRID/demo/make_cubrid_demo.sh" ]; then
        echo "Seeding demodb using make_cubrid_demo.sh ..."
        gosu cubrid "$CUBRID/demo/make_cubrid_demo.sh"
        echo "Seed done."
        return 0
    fi

    echo "Demo script not found: $CUBRID/demo/make_cubrid_demo.sh"
    return 1
}
    ...
    if [ $# -eq 0 ]; then
        case "$CUBRID_COMPONENTS" in
            ...
            ALL)
                ...
                ;;
            DEMO)
                echo "Starting CUBRID Demodb..."
                if seed_demodb; then
                    gosu cubrid cubrid broker start && gosu cubrid cubrid server start demodb
                else
